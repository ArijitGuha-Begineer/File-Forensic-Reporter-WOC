...
def xxd(loc):
   st=subprocess.Popen([f'xxd -a {loc} | head'],stdout=subprocess.PIPE,shell=True)
   output=st.communicate()[0]
...
def stegsnow(loc):
   st=subprocess.Popen(['stegsnow',loc,'snowoutput.txt'],stdout=subprocess.PIPE)
   output=st.communicate()[0]
   with open("Report.md","a")as f:
      f.write("\n\n*******output from stegsnow*******\n\n")
      f.close
   try:
      with open("snowoutput.txt","r")as l:
         text=l.read()
      with open("Report.md","a")as f:
         f.write(text)
         f.close()
